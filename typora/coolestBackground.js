function coolestBackground(s_container,config){
    ...
    if(config){
        if(config.MESH){
            for(key in config.MESH){
                MESH[key]=config.MESH[key]
            }
        }
        if(config.LIGHT){
            for(key in config.LIGHT){
                LIGHT[key]=config.LIGHT[key]
            }
        }
    }else {
        config={};
    ...
    if(config.mode.display){
        RENDER.renderer=config.mode.display
    }
    //------------------------------
    // Export Properties
    //------------------------------
